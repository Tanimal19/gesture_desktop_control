...
from src.ui.dc_canva import ArrowElement, DotElement
import random
...
class TrueTaskType(Enum):
    BA_POINT = "basic_point"
    BA_LEFT_CLICK = "basic_left_click"
    BA_RIGHT_CLICK = "basic_right_click"
    BA_SCROLL_UP = "basic_scroll_up"
    BA_SCROLL_DOWN = "basic_scroll_down"
    MENU_NAVIGATION = "menu_navigation"
    DRAGGING = "dragging"
    POINT_N_CLICK = "point_and_click"


def return_tclass(task_type: str):
    if task_type == TrueTaskType.BA_POINT.value:
        return BasicPointTask
    elif task_type == TrueTaskType.BA_LEFT_CLICK.value:
        return BasicLeftCLickTask
    elif task_type == TrueTaskType.BA_RIGHT_CLICK.value:
        return BasicRightClickTask
    elif task_type == TrueTaskType.BA_SCROLL_UP.value:
        return BasicScrollUpTask
    elif task_type == TrueTaskType.BA_SCROLL_DOWN.value:
        return BasicScrollDownTask
    elif task_type == TrueTaskType.MENU_NAVIGATION.value:
        return MenuNavigationTask
    elif task_type == TrueTaskType.DRAGGING.value:
        return DraggingTask
    elif task_type == TrueTaskType.POINT_N_CLICK.value:
        return PointAndClickTask
    else:
        raise ValueError("Unsupported task type")


class BasicPointTask:
...
class BasicScrollUpTask:
    name = "Basic Scroll Up Task"
    instruction = "Swing your index and middle fingers upward, repeat 5 times."
...
class BasicScrollDownTask:
    name = "Basic Scroll Down Task"
    instruction = "Swing your index and middle fingers downward, repeat 5 times."
    ...
    def generate_configs(count, canva_bound):
        ...
        line_length = 600
        for _ in range(count):
            x1 = canva_bound[0] + (canva_bound[1] - canva_bound[0]) // 2
            y1 = (
                canva_bound[2]
                + (canva_bound[3] - canva_bound[2]) // 2
                - (line_length // 2)
            )
            x2 = x1
            y2 = y1 + line_length
